...
resp_scale = c(mean(final_train_nn$new_cases_log), sd(final_train_nn$new_cases_log))
...
complete_nn = final_train_nn %>% rbind(final_test_nn) %>%
  ...
  group_by(location) %>%
  select(c(date, new_cases, new_cases_log, location, new_deaths,
           new_tests, new_tests_b, gdp_per_capita)) %>%
  ungroup()
...
data_norm = complete_nn %>%
  mutate(
    new_cases_log = as.vector(scale(new_cases_log)),
  ...
  mutate(
    # new_cases
    new_cases_log_lag7 = lag(new_cases_log, 7, default = NA),
    # location
...
data_train_x[,,1] = as.matrix(train_nn %>% select(new_cases_log_lag7))
...
data_train_y[,,1] = train_nn$new_cases_log
...
data_test_x[,,1] = as.matrix(test_nn %>% select(new_cases_log_lag7))
...
data_test_y[,,1] = test_nn$new_cases_log
...
final_train = train_nn %>% mutate(pred = exp(fitted_train))
results = final_train %>%
  group_by(location) %>%
...
final_test = test_nn %>% mutate(pred = exp(fitted_test))
