...
train_prophet  <- data_lm |> arrange(date) %>% filter(date < as.Date("2023-01-01"))
test_prophet <- data_lm |> arrange(date) %>% filter(date >= as.Date("2023-01-01"))

write_rds(train_prophet, "data/finalized_data/train_prophet.rds")
write_rds(test_prophet, "data/finalized_data/test_prophet.rds")
