...
# Imputation for large column missingness: Neural Network = create booleann indicators
data_nn2 = data_nn %>%
  mutate(
    icu_patients_b = ifelse(is.na(icu_patients) & (date < as.Date("2021-02-01") | date > as.Date("2022-03-01")),
                            FALSE, TRUE),
    hosp_patients_b = ifelse(is.na(hosp_patients) & (date < as.Date("2021-02-01") | date > as.Date("2022-03-01")),
                             FALSE, TRUE),
    total_tests_b = ifelse(is.na(total_tests) & (date < as.Date("2021-02-01") | date > as.Date("2022-03-01")),
                           FALSE, TRUE),
    new_tests_b = ifelse(is.na(new_tests) & (date < as.Date("2021-02-01") | date > as.Date("2022-03-01")),
                         FALSE, TRUE),
    positive_rate_b = ifelse(is.na(positive_rate) & (date < as.Date("2021-02-01") | date > as.Date("2022-03-01")),
                             FALSE, TRUE),
    total_vaccinations_b = ifelse(is.na(total_vaccinations) & (date < as.Date("2021-02-01") | date > as.Date("2022-03-01")),
                                  FALSE, TRUE),
    people_vaccinated_b = ifelse(is.na(people_vaccinated) & (date < as.Date("2021-02-01") | date > as.Date("2022-03-01")),
                                 FALSE, TRUE)
  )
...
data_nn3 = data_nn3 %>%
  ...
  ungroup()
# NOTE: hosp_patients and icu_patients have missingness only due to countries not reporting
